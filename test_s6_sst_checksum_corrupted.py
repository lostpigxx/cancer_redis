# test_s6_sst_checksum_corrupted.py
...
def test_s6_sst_checksum_corrupted_and_repair():
    """
    S6：SST data block trailer checksum 区域损坏。

    用例自己构造 SST：
      write -> flushmem -> wait new SST -> corrupt SST checksum area
    """
    ctx = RepairAT()
    ...
    # ---------- T5：关闭 heartbeat，停止目标 shardsvr，损坏 SST checksum ----------
    ...
    with ctx.heartbeat_disabled():
        ...
        print("selected live SST file to corrupt checksum: {}".format(sst_file))

        ctx.corrupt_sst_checksum_area(sst_file)
