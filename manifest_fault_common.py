...
def run_manifest_fault_case(case_name, inject_fault):
    ctx = RepairAT()
    ...
    # ---------- T3: prepare target SST data and current MANIFEST ----------
    ...
    print("target SST files generated before MANIFEST fault: {}".format(new_ssts))
    ...
    with ctx.heartbeat_disabled():
        ...
        print(
            "injected MANIFEST fault: case={}, description={}".format(
                case_name,
    ...
    # MANIFEST metadata faults may make repair rebuild metadata and drop target
