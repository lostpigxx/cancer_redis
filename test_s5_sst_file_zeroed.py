...
def test_s5_sst_file_zeroed_and_repair():
    """
    S5：SST 文件名存在但内容清零。

    用例自己构造 SST：
      write -> flushmem -> wait new SST -> zero SST content
    """
    ctx = RepairAT()
    ...
    # ---------- T5：关闭 heartbeat，停止目标 shardsvr，清零 SST ----------
    ...
    with ctx.heartbeat_disabled():
        ...
        print("selected live SST file to zero: {}".format(sst_file))

        ctx.zero_sst_file(sst_file)
