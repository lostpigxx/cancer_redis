# test_s9_sst_index_block_corrupted.py
...
def test_s9_sst_index_block_corrupted_and_repair():
    """
    S9：SST index block 损坏。

    用例自己构造 SST：
      write -> flushmem -> wait new SST -> corrupt SST index block
    """
    ctx = RepairAT()
    ...
    # ---------- T5：关闭 heartbeat，停止目标 shardsvr，损坏 SST index block ----------
    ...
    with ctx.heartbeat_disabled():
        ...
        print("selected live SST file to corrupt index block: {}".format(sst_file))

        ctx.corrupt_sst_index_block_area(sst_file)
