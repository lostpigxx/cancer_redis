# test_m11_current_truncated_manifest_name

## M11：CURRENT 中 MANIFEST 文件名被截断流程图

```text
[准备阶段]

以下每个子场景都会独立执行完整流程：CURRENT 写成 `MANIFEST-` 或 `MANIFEST-000`。
...
临时关闭 heartbeat
  ...
  -> 按当前子场景将 CURRENT 写成不完整的 MANIFEST 文件名
  -> 在原位置重新启动这个 shardsvr
  -> 恢复 heartbeat
