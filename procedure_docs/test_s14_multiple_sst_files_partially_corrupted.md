# test_s14_multiple_sst_files_partially_corrupted

## S14：多个 SST 同时部分损坏流程图
...
开启 heartbeat
  ...
  -> 写入一批适合 data block 损坏的目标数据并 flush 成 SST
  -> 再写入一批适合 footer 损坏的目标数据并 flush 成 SST
...
临时关闭 heartbeat
  ...
  -> 选择两个目标 SST：一个损坏 footer，一个损坏 data block 区域
  -> 在原位置重新启动这个 shardsvr
  -> 恢复 heartbeat
