# test_m16_manifest_sst_metadata_tampered

## M16：MANIFEST 中 SST 元数据被篡改流程图

```text
[准备阶段]

以下每个参数化子场景都会独立执行完整流程：缺失 SST 引用、错误文件号、错误 level、错误 key range。

确认已配置外部 MANIFEST 元数据篡改工具
  -> 开启 heartbeat
  -> 确认所有 partition 正常 opened
  -> 选择一个目标 partition，并记住它当前在哪个 shardsvr 上
  -> 写入目标 partition 的测试数据
  -> flush 目标 partition，生成目标 SST 和当前 MANIFEST
...
临时关闭 heartbeat
  ...
  -> 选择目标 SST，并按当前子场景用外部工具篡改 MANIFEST 中的 SST 元数据
  -> 在原位置重新启动这个 shardsvr
  -> 恢复 heartbeat
